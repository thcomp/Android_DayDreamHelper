...
import jp.co.thcomp.glsurfaceview.GLSurfaceObserver;
import jp.co.thcomp.util.HashMultimap;
import jp.co.thcomp.util.LogUtil;

abstract public class AbstractDDRenderView extends GvrView implements View.OnTouchListener {
    ...
    protected DDSurfaceObserver mObserver;
    ...
    public AbstractDDRenderView(Context context) {
        this(context, null);
    ...
    public AbstractDDRenderView(Context context, AttributeSet attrs) {
        super(context, attrs);
        if(LogUtil.isOutput(Constant.LOG_TYPE.LOG_TYPE_DEBUG)){
            setDebugFlags(DEBUG_LOG_GL_CALLS | DEBUG_CHECK_GL_ERROR);
        }
    ...
    public void initialize(GL10 gl, int width, int height){
        ...
        mGLContext.setViewport(gl, 0, 0, width, height);
        mGLContext.setGLConfiguration(gl);
